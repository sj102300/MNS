...
int main(void) {
    ...
    for (int i = 0; i < id.size(); i++) {
        string command = "\"C:\\Users\\user\\Desktop\\lig2_project\\x64\\Debug\\lig2_project.exe\" "
            + id[i] + " " + to_string(s_x[i]) + " " + to_string(s_y[i]) + " " + to_string(z[i]) + " " + to_string(f_x[i]) + " " + to_string(f_y[i]);
